...
def find_recipe(recipe):  
    ...
    file = glob.glob( path+'//recipes//*//' + recipe + '*.json')

    if len(file)> 0:
        out_file = file[0]

    elif recipe[-4:] =='json':
        # remote list with other groceries  
        out_file = recipe
